...
impl RuntimeKeymap {
    ...
    pub(crate) fn from_config(keymap: &TuiKeymap) -> Result<Self, String> {
        ...
        let insert_newline = if keymap.editor.insert_newline.is_some() {
            resolve_local!(keymap, defaults, editor, insert_newline)
        } else if keymap.composer.submit.is_some() || keymap.global.submit.is_some() {
            defaults
                .editor
                .insert_newline
                .iter()
                ...
                .filter(|binding| {
                    *binding == key_hint::plain(KeyCode::Enter)
                        || !composer.submit.contains(binding)
                })
                .collect()
        } else {
            defaults.editor.insert_newline.clone()
        };
